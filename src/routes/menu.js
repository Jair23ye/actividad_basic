...
router.get('/restaurante/:restaurante_id', (req, res) => {
    const { restaurante_id } = req.params;
    
    const sql = 'SELECT * FROM menu WHERE restaurante_id = ? ORDER BY nombre ASC';
    
    conection.query(sql, [restaurante_id], (err, results) => {
        if (err) {
            console.error('Error al obtener el menú del restaurante:', err);
            return res.status(500).json({ error: 'Error interno al obtener el menú' });
        }
        res.json(results);
    });
});
